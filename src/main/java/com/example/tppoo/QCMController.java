...
public class QCMController implements Initializable {
    ...
    public void displayQCMQuestionsInTable() {
        List<QCM> qcmQuestions = this.Test.getTest().getQCMQuestions();
        if (qcmQuestions != null) {
            ObservableList<QCM> qcmList = FXCollections.observableArrayList(qcmQuestions);
            table.setItems(qcmList);
        } else {
            // Handle the case when QCMQuestions is null, such as displaying an error message
            System.out.println("No QCM questions found.");
        }
    }
